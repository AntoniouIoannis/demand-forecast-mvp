#!/usr/bin/env python3
"""
Train a demand model and produce MONTHLY metrics (timerange=month) on the test set.
...
def add_safe_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Assumes df already has ordered_lag1 and ordered rolling stats from preprocess.
    """
...
def main() -> None:
    ...
    # Concatenate so lags for early test months use prior months (valid, since they are in the past)
